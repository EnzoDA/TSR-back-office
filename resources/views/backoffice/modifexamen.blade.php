...
<section class="main-container">
    <div class="title-style">
        <p>Modification d&#039un examen</p>
    </div>
    <form action="{{route('modifexamenencours')}}" method="post" class="form-style">
        @csrf
        <label for="repere" class="element-style">Code repère :</label>
        <input type="text" name="repere" required class="element-style-input" placeholder="{{$ex->repere}}">
        ...
        <input type="text" name="commentaire" class="element-style-input" pattern="[A-z\s\é\à\è\ç\ù\']{1,250}" title="Seulement des lettres" placeholder="{{$ex->commentaire}}">
        <label for="regle" required class="element-style">Règle :</label>
        <input type="text" name="regle" pattern="[A-z\s\é\à\è\ç\ù\']{1,350}" title="Seulement des lettres" required class="element-style-input" placeholder="{{$ex->regle}}">
